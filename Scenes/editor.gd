	...
	if !globalEditor.popupIsOpen:
		if globalEditor.isEditing and !globalEditor.popupIsOpen:
			...
			if (cursorCellCoords!=previousCursorCellCoords or (round(camera.controllablePhantomCamera2D.position*100) != round(camera.position*100)) ) and selectedItem is terrainItem:
				...
				cursorItemIcon.position = cursorCanvas.cursor.cursorSprite.position-get_viewport().canvas_transform.origin - (Vector2.ONE * (cursorItemIcon.texture.get_size()/2) if selectedItem.centerPreview else Vector2.ZERO) + selectedItem.textureOffset
...
#		If Not editing
	...
	cursorItemIcon.visible = cursorCanvas.cursor.visible and globalEditor.isEditing and !globalEditor.popupIsOpen and globalEditor.currentTool==globalEditor.Tools.place
