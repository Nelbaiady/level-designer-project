...
func _on_task_completed(result : int, response_code : int, headers : PackedStringArray, body : PackedByteArray, handler: HTTPRequest) -> void:
	...
	if(!body.is_empty()):
		result_body = JSON.parse_string(body.get_string_from_utf8().strip_edges(true, true))
		if body.get_string_from_utf8().strip_edges(true, true) != "":
			var parsed = JSON.parse_string(body.get_string_from_utf8().strip_edges(true, true))
			if typeof(parsed) == TYPE_DICTIONARY:
				result_body = parsed
			else:
				# JSON failed, leave result_body empty
				result_body = {}
