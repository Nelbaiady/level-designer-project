...
func ensureAccountExists():
	print("checking for user "+sessionCache.user.id)
	var response = await rpcRequest({},"getCurrentUser")
	var body = response[3]
	
	#if there is no instance of this user
	if body.get_string_from_utf8() == "[]" or body.get_string_from_utf8()=="null":
		print("user doesnt exist. adding now...")
		await signUp()
	else:
...
##Response[3]: body[br]
func rpcRequest(params:Dictionary,functionName:String, signedIn:=true):
